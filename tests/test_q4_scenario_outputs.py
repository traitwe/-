...
def test_scenario_comparison_reports_baseline_gap_and_reoptimised_plan():
    ...
    vot = {"vot_scenario": "central", "vot_cny_per_hour": 20.0, "parking_delay_hours": 0.5, "shuttle_delay_hours": 0.25, "staff_delay_hours": 0.1}
    result = compare_baseline_with_reoptimisation(baseline, shocked, scenario, vot_parameters=vot)

    assert result.loc[0, "baseline_service_gap"] > 0
    assert result.loc[0, "baseline_traveler_time_loss_cny"] > 0
    assert result.loc[0, "reoptimized_shuttle_vehicles"] >= result.loc[0, "baseline_shuttle_vehicles"]
    assert result.loc[0, "reoptimized_traveler_time_loss_cny"] >= 0
