...
def independent_static_tiers(historical_peak_relative_pressure: float, safety_factor: float = 1.2) -> dict[str, int]:
    """Return a fixed baseline from historic peak pressure, not optimized output."""
    if historical_peak_relative_pressure < 0 or safety_factor < 1:
        raise ValueError("historical peak pressure must be non-negative and safety factor at least one")
    safeguarded_pressure = historical_peak_relative_pressure * safety_factor
    return {f"{resource}_tier": _required_tier(safeguarded_pressure, resource) for resource in RESOURCE_NAMES}


def optimize_relative_tiers(relative_pressure: float, risk_penalty: float = 5.0, budget: float | None = None, anchor_support: dict[str, float] | None = None, minimum_tiers: dict[str, int] | None = None) -> tuple[dict[str, float | int], pd.DataFrame]:
    ...
    minimum = {resource: 0 for resource in RESOURCE_NAMES}
    for resource, value in (minimum_tiers or {}).items():
        if resource not in minimum or int(value) != value or not 0 <= int(value) <= 2:
            raise ValueError("minimum tiers must use known resources and integers in [0, 2]")
        minimum[resource] = int(value)
    ...
    for resource, level in minimum.items():
        diagnostics = diagnostics.loc[diagnostics[f"{resource}_tier"].ge(level)].copy()
    if budget is not None:
        diagnostics = diagnostics.loc[diagnostics["relative_cost"] <= budget].copy()
    if diagnostics.empty:
        raise ValueError("no relative tier configuration satisfies the minimum-tier and budget constraints")
