...
为避免上述单一年份留出法偶然有利于某一模型，另以 2024-04-01 和 2024-07-01 为滚动原点，各进行 92 日前向检验；每个原点的训练、锚点尺度换算与区间校准均严格截断在原点之前。动态 Ridge 在全片区窗口的 WAPE 分别为 37.63\%（$n=36$）和 70.89\%（$n=83$），均低于季节--日历 Ridge 的 39.30\% 和 75.53\%。7--8 月子样本（$n=65$）的 WAPE 为 65.92\%。该附加检验仅条件于当期已实现的天气和滞后搜索量，且被解释变量仍是景区锚点约束指数，不将其宣传为事前实测客流预测精度。
...
问题二可复核表为 \url{q2_daily_rolling_origin_validation.csv}（位于 \url{outputs/question2_analysis}）。其固定尝试 2024-01-01、2024-04-01、2024-07-01 三个原点和 92 日窗口；原点前的数据才可进入训练、游客指数--压力尺度换算与 split-conformal 校准。若某窗口去重后少于 20 条锚点观测，则不输出该窗口，而不以插补值补足。动态日模型只使用日历、当期天气和预先滞后的搜索变量，不使用 \texttt{target\_lag\_1} 或任何未来游客量。滚动检验中的天气与搜索量是已实现协变量，仅用于条件检验；2026 年预测改用预预测期按月日构造的气候情景。
\end{appendices}
