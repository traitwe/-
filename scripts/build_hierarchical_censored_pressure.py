...
panel = pd.read_csv(ROOT / 'data/model_input/censored_attraction_observation_panel.csv', encoding='utf-8-sig')
target_regions = ['BDH', 'HGA', 'SHG']
panel = panel.loc[panel['region_code'].isin(target_regions)].copy()
calendar = pd.read_csv(ROOT / 'data/clean/calendar_2015_2026.csv')
weather = pd.read_csv(ROOT / 'data/clean/daily_weather_2015_2025.csv')
search = pd.read_csv(ROOT / 'data/model_input/daily_search_theme_features_2016_2026.csv')
...
out.to_csv(ROOT / 'data/model_input/daily_region_censored_likelihood_pressure_2023_2025.csv', index=False, encoding='utf-8-sig')
fit['sampled_pressure'].to_csv(ROOT / 'data/model_input/censored_likelihood_sampled_pressure.csv', index=False, encoding='utf-8-sig')
fit['parameter_diagnostics'].to_csv(ROOT / 'data/model_input/censored_likelihood_parameter_diagnostics.csv', index=False, encoding='utf-8-sig')
baseline = pd.read_csv(ROOT / 'data/model_input/daily_region_pressure_baseline_2023_2025.csv', encoding='utf-8-sig')
...
comparison.to_csv(ROOT / 'data/model_input/censored_likelihood_vs_ridge_validation.csv', index=False, encoding='utf-8-sig')
report={
...
(ROOT / 'data/model_input/censored_likelihood_quality_report.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8')
